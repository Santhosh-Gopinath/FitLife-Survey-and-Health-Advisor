...
base_dir = r"D:\SEM 6\Project\Fitnexus\try7\client"
...
diets = pd.read_csv(os.path.join(datasets_dir, "diets.csv"))  # Fixed path for diets.csv
...
# Symptoms dictionary for model input
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            # Filter out symptoms not in the dictionary
            valid_symptoms = [symptom for symptom in user_symptoms if symptom in symptoms_dict]
            
            if not valid_symptoms:
                message = "No valid symptoms provided. Please check your input."
                return render_template('index1.html', message=message)
